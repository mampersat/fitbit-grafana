...
6. Finally run : `docker compose up -d` ( to launch the full stack in detached mode ). Thereafter you should check the logs with `docker compose logs --follow` to see any potential error from the containers. This will help you debug the issue, if there is any (specially read/write permission issues). This step will automatically create the influxdb folder; if you encounter a permission error preventing the container from starting, fix it by running `chown -R 1500:1500 influxdb`, as the official InfluxDB image for v1.11.x runs under UID/GID 1500 and requires proper ownership of that directory.
...
- In the docker compose file, modify the ENV variable to `AUTO_DATE_RANGE=False` under the `environment` section along with other variables. This variable switches the mode to bulk update instead of regular daily update
